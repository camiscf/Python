códigos_para_símbolos = {111: '0', 112: '1', 113: '2', 114: '3', 115: '4',
116: '5', 117: '6', 118: '7', 119: '8', 121: '9', 122: '=', 123: '+',
124: '-', 125: '/', 126: '*', 127: 'a', 128: 'b', 129: 'c', 131: 'd',
132: 'e', 133: 'f', 134: 'g', 135: 'h', 136: 'i', 137: 'j', 138: 'k',
139: 'l', 141: 'm', 142: 'n', 143: 'o', 144: 'p', 145: 'q', 146: 'r',
147: 's', 148: 't', 149: 'u', 151: 'v', 152: 'w', 153: 'x', 154: 'y',
155: 'z', 156: 'á', 157: 'à', 158: 'â', 159: 'ã', 161: 'é', 162: 'ê',
163: 'í', 164: 'ó', 165: 'ô', 166: 'õ', 167: 'ú', 168: 'ç', 169: 'A',
171: 'B', 172: 'C', 173: 'D', 174: 'E', 175: 'F', 176: 'G', 177: 'H',
178: 'I', 179: 'J', 181: 'K', 182: 'L', 183: 'M', 184: 'N', 185: 'O',
186: 'P', 187: 'Q', 188: 'R', 189: 'S', 191: 'T', 192: 'U', 193: 'V',
194: 'W', 195: 'X', 196: 'Y', 197: 'Z', 198: 'Á', 199: 'À', 211: 'Â',
212: 'Ã', 213: 'É', 214: 'Ê', 215: 'Í', 216: 'Ó', 217: 'Ô', 218: 'Õ',
219: 'Ú', 221: 'Ç', 222: ',', 223: '.', 224: '!', 225: '?', 226: ';',
227: ':', 228: '_', 229: '(', 231: ')', 232: '"', 233: '#', 234: '$',
235: '%', 236: '@', 237: ' ', 238: '\n'}

símbolos_para_códigos = {'0': 111, '1': 112, '2': 113, '3': 114, '4': 115,
'5': 116, '6': 117, '7': 118, '8': 119, '9': 121, '=': 122, '+': 123,
'-': 124, '/': 125, '*': 126, 'a': 127, 'b': 128, 'c': 129, 'd': 131,
'e': 132, 'f': 133, 'g': 134, 'h': 135, 'i': 136, 'j': 137, 'k': 138,
'l': 139, 'm': 141, 'n': 142, 'o': 143, 'p': 144, 'q': 145, 'r': 146,
's': 147, 't': 148, 'u': 149, 'v': 151, 'w': 152, 'x': 153, 'y': 154,
'z': 155, 'á': 156, 'à': 157, 'â': 158, 'ã': 159, 'é': 161, 'ê': 162,
'í': 163, 'ó': 164, 'ô': 165, 'õ': 166, 'ú': 167, 'ç': 168, 'A': 169,
'B': 171, 'C': 172, 'D': 173, 'E': 174, 'F': 175, 'G': 176, 'H': 177,
'I': 178, 'J': 179, 'K': 181, 'L': 182, 'M': 183, 'N': 184, 'O': 185,
'P': 186, 'Q': 187, 'R': 188, 'S': 189, 'T': 191, 'U': 192, 'V': 193,
'W': 194, 'X': 195, 'Y': 196, 'Z': 197, 'Á': 198, 'À': 199, 'Â': 211,
'Ã': 212, 'É': 213, 'Ê': 214, 'Í': 215, 'Ó': 216, 'Ô': 217, 'Õ': 218,
'Ú': 219, 'Ç': 221, ',': 222, '.': 223, '!': 224, '?': 225, ';': 226,
':': 227, '_': 228, '(': 229, ')': 231, '"': 232, '#': 233, '$': 234,
'%': 235, '@': 236, ' ': 237, '\n': 238}


from random import randrange
from typing import cast


def impar(m):
  k = 0
  q = m
  while q%2 == 0:
    k += 1
    q //= 2
  return k,q

def teste_Miller_Rabin(p,b):
 
  k, q = impar(p-1)
  
  x = pow(b, q, p) 
  if x == 1 or x == p-1:
    return "teste inconclusivo"

  contador = 0
  while True:
    x = pow(x,2,p)
    contador += 1
    if x == 1:
      return "composto"
    if contador == k:
      return "composto"
    if x == p-1:
      return "teste inconclusivo"

def gerar_primos(a):
  inconclusivo = 0

  p = randrange(10**a,10**(a+2))
  while p % 2 == 0:
        p = randrange(10**a,10**(a+2))
  count = 0
  while count != 10:
    b = randrange(1,p-1)
    if teste_Miller_Rabin(p,b) == "teste inconclusivo":
        inconclusivo +=1
    else:
      p = randrange(10**a,10**(a+2))
    while p % 2 == 0:
        p = randrange(10**a,10**(a+2))
        count = 0
    count += 1     
  return p

#euclides para inverter

def euclides(a, b):
    x_antigo = y_novo = 1
    x_novo = y_antigo = 0
    Dividendo, Divisor = a, b
    while Divisor != 0:
        Quociente, Resto = divmod(Dividendo, Divisor)
        x_antigo, x_novo = x_novo, x_antigo - x_novo*Quociente
        y_antigo, y_novo = y_novo, y_antigo - y_novo*Quociente
        Dividendo, Divisor = Divisor, Resto
    if Dividendo >= 0:
        return Dividendo, x_antigo, y_antigo
    return -Dividendo, -x_antigo, -y_antigo

#vai gerar as chaves 

def gera_chaves(a):
    p = gerar_primos(a)
    q = gerar_primos(a)
    n=p*q
    phi = (p-1)*(q-1)
    while(True): 
        e = randrange(3,100)
        mdc, d, _ = euclides(e,phi)
        if mdc == 1: break
        
    _,inversoP, inversoQ = euclides(p,q)
   # _, d, _ = euclides(e,phi)
    reduzidaDmodP = pow(d,1,p-1)
    reduzidaDmodQ = pow(d,1,q-1)
    return n, p, q, phi, e, d, inversoP, inversoQ, reduzidaDmodP, reduzidaDmodQ

#encriptar a mensagem com a precodificação e elevar 
def encriptar(texto, n,e):
    j=0
    letra = []
    blocos = []
    blocos_final = []
    acabou = []

    for caractere in texto:
        codigo = str(símbolos_para_códigos[caractere])
        blocos.append(codigo)
    junta_tudo = ''.join(blocos)

    for i in range(len(junta_tudo)):
        if ((i-j))>=len(str(n//10**2)):
            blocos_final.append(junta_tudo[j:i])
            j = i
    blocos_final.append(junta_tudo[j:])

    for salvei in blocos_final:
        temp = pow(int(salvei),e,n)
        acabou.append(temp)
    return acabou
#descriptar a mensagem 
def descriptar(blocos,n,d):
    blocos_final = []
    lista_final = []
    string_vazia = ''

    for bloquinhos in blocos:
        cansei = pow(int(bloquinhos),d,n)
        blocos_final.append(cansei)

    for bloco in blocos_final:
        j = 0

        bloco = str(bloco)
       
        string_vazia += bloco
    for i in range (len(string_vazia)):
        if (i+1) % 3 == 0:

            lista_final.append(códigos_para_símbolos[int(string_vazia[j:i+1])])
            j = i+1

    return ''.join(lista_final)
